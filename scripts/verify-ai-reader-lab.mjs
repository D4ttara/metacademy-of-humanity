...
for(const [n,issue,slug,pages] of expected) for(const file of pages){
 ...
 for(const token of [`id="ai-reader-lab-${n}"`,`data-ai-reader-lab="${n}"`,'data-ai-reader-mode="tailored"','data-ai-reader-jump',`href="#ai-reader-lab-${n}"`,'data-copy-ai-prompt','data-share-reader','data-ai-prompt','ai-reader-lab.js',`issues/${issue}`]) assert(h.includes(token),`${file} missing ${token}`);
...
let readerPages=0;
for(const file of walk('documents')){const h=readFileSync(file,'utf8'); const m=h.match(/data-reader-document="(\d{3})"/); if(!m) continue; readerPages++; const n=m[1]; assert(h.includes(`data-ai-reader-lab="${n}"`),`${file} is reader-enabled but AI Reader Lab is missing`); assert(h.includes('data-copy-ai-prompt')&&h.includes('data-share-reader'),`${file} missing zero-service AI controls`);}
...
console.log(`AI_READER_LAB_VERIFY=PASS publications=${expected.length} editions=${editions} reader_pages=${readerPages} discovery_checks=${discovery} tailored=PASS future_reader_guard=PASS copy=PASS share=PASS critical_prompts=PASS backend=ZERO`);
