...
for(const p of ['robots.txt','sitemap.xml','llms.txt','llms-full.txt','ai-index.json','feed.xml','identity.json']) must(existsSync(p),`missing discovery surface: ${p}`);
...
for(const required of [SITE+'/',SITE+'/uk/',SITE+'/manifesto/',SITE+'/documents/',SITE+'/topics/',SITE+'/identity/',SITE+'/uk/books/memories-of-humanity/book-1/free-reading/',SITE+'/uk/books/memories-of-humanity/book-1/free-reading/skuf-review/',SITE+'/uk/books/memories-of-humanity/book-1/free-reading/esthete-review/']) must(urls.has(required),`AI index missing ${required}`);
...
for(const required of [SITE+'/manifesto/',SITE+'/documents/',SITE+'/topics/',SITE+'/uk/',SITE+'/identity/']) must(full.includes(required),`llms-full missing ${required}`);
